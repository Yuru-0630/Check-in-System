<?php
  ...
  if($user_identity==2)
  {
    echo '<!DOCTYPE html>
              ...
              <main class="main">
                ...
                <div class="container-fluid">
                  <div class="card">
                    ...
                    <div class="card-body">
                      <a href="add_class_page.php">新增今日課程</a>
        ...
        $index_page = $_GET['i'];
        $num_of_data = get_Num_of_Info("class");
        $num_of_page = (int)($num_of_data/$data_length_one_page);
        if($num_of_data%$data_length_one_page!=0)
        {
          $num_of_page++;
        }
        $result = get_Info_desc_in_range("class",1+($index_page-1)*$data_length_one_page,$data_length_one_page+($index_page-1)*$data_length_one_page);
        while($array = mysqli_fetch_assoc($result))
        {
          echo '<tr>
                  <td>'.$array['ID'].'</td>
                  <td class="text-center">'.$array['class_date'].'</td>
                  <td class="text-center">'.$array['starting_time'].'</td>
                  <td class="text-center">'.$array['ending_time'].'</td>
                  <td class="text-center">'.$array['note'].'</td>
                  <td class="text-center">
                  <button onclick="location.href=\'class_attendance.php?i=1&id='.$array['ID'].'\'">查看</button>
                  <button onclick="delete_class('.$array['ID'].');">刪除</button>
                  </td>
                </tr>';
                $list_index += 1;
        }
          ...
          paging("attendance.php",$index_page,$num_of_page,$id);
          echo '</ul>
              </nav>
            </div>
          </div>
